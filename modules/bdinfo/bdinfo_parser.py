...
class BDInfoParser:
    def __init__(self, *, bdinfo_script: str, upload_media: str):
        self.bd_max_file = None
        self.largest_playlist = None
    ...
    def _validate_environment(self):
        ...
        if UploadAssistantConfig().CONTAINERIZED or os.path.isfile(self.bdinfo_script):
            return

        logging.critical(
            "[BDInfoParser] Missing bdinfo script path in config.env for '-disc' arg."
        )
        logging.info(
            "[BDInfoParser] Can not upload a raw disc without bdinfo, update the 'bdinfo_script' path in config.env"
        )
        raise GGBotUploaderException(
            f"The bdinfo script you specified: ({self.bdinfo_script}) does not exist"
        )

    def _validate_bdmv_stream(self):
        if os.path.exists(f"{self.upload_media}BDMV/STREAM/"):
            return

        logging.critical(
            "[BDInfoParser] BD folder not recognized. We can only upload if we detect a '/BDMV/STREAM/' folder"
        )
        raise GGBotUploaderException(
            "Currently unable to upload .iso files or disc/folders that does not contain a '/BDMV/STREAM/' folder"
        )

    def get_largest_playlist(self, auto_mode: bool = False):
        bd_max_size = 0
        ...
        logging.debug(
            f"[BDInfoParser] BDInfo output split from of list command: ---{bdinfo_output_split}--- "
        )
        ...
        for index, mpls_playlist in enumerate(bdinfo_output_split):
            if mpls_playlist not in all_mpls_playlists:
                continue
            ...
            playlist_details = {
                ...
                "est_bytes": bdinfo_output_split[index + 2],
                "msr_bytes": bdinfo_output_split[index + 3],
            }
            try:
        ...
        if not auto_mode:
            ...
            logging.debug(
                f"[BDInfoParser] Used decided to select the playlist [{largest_playlist}] with Playlist # [{user_input_playlist_id_num}]"
            )
            logging.info(
                f"[BDInfoParser] Largest playlist obtained from bluray disc: {largest_playlist}"
            )
            self.bd_max_file = bd_max_file
            self.largest_playlist = largest_playlist
        else:
            ...
            self.bd_max_file = bd_max_file
            self.largest_playlist = largest_playlist
        return self.bd_max_file, self.largest_playlist

    def generate_and_parse_bdinfo(
        self, torrent_info: Dict[str, Any], debug: bool = False
    ):
        """
        ...
        logging.debug(
            f"[BDInfoParser] `largest_playlist` and `upload_media` from torrent_info :: {torrent_info['largest_playlist']} --- {torrent_info['upload_media']}"
        )
        subprocess.run(
            [
                self.bdinfo_script,
        ...
        if debug:
            logging.debug(
                "[BDInfoParser] ::::::::::::::::::::::::::::: Dumping the BDInfo Quick Summary :::::::::::::::::::::::::::::"
            )
        GenericUtils.write_file_contents_to_log_as_debug(torrent_info["mediainfo"])
        return self.parse_bdinfo(torrent_info["mediainfo"])
    ...
    @staticmethod
    def get_video_codec_from_bdinfo(bdinfo):
        """
        ...
        for index, video_track in enumerate(bdinfo["video"]):
            if "dv_hdr" in video_track and len(video_track["dv_hdr"]) != 0:
                ...
                logging.debug(
                    f"[BDInfoParser] Detected {video_track['dv_hdr']} from bdinfo in track {index}"
                )
                if (
                    ...
                    if "HDR10+" in hdr:
                        ...
                        hdr = "HDR"
                    logging.debug(
                        f"[BDInfoParser] Adding proper HDR Format `{hdr}` to torrent info"
                    )
        logging.info(
            f"[BDInfoParser] `video_codec` identified from bdinfo as {bdinfo['video'][0]['codec']}"
        )
        # video codec is taken from the first track
        return dv, hdr, bdinfo["video"][0]["codec"], bdinfo["video"][0]["codec"]

    @staticmethod
    def get_audio_codec_from_bdinfo(bdinfo, audio_codec_dict):
        """
        ...
        for audio_track in bdinfo["audio"]:
            if "atmos" in audio_track and len(audio_track["atmos"]) != 0:
                logging.info(
                    f"[BDInfoParser] `atmos` identified from bdinfo as {audio_track['atmos']}"
                )
                atmos = "Atmos"
                break

        logging.info(
            f"[BDInfoParser] `audio_codec` identified from bdinfo as {bdinfo['audio'][0]['codec']}"
        )
        for key in audio_codec_dict.keys():
            if str(bdinfo["audio"][0]["codec"].strip()) == key:
                logging.info(
                    f'[BDInfoParser] Used (audio_codec_dict + BDInfo) to identify the audio codec: {audio_codec_dict[bdinfo["audio"][0]["codec"].strip()]}'
                )
                return atmos, audio_codec_dict[bdinfo["audio"][0]["codec"].strip()]
        logging.error(
            f"[BDInfoParser] Failed to identify audio_codec from audio_codec_dict + BDInfo. Audio Codec from BDInfo {bdinfo['audio'][0]['codec']}"
        )
        return None, None

    @staticmethod
    def get_audio_channels_from_bdinfo(bdinfo):
