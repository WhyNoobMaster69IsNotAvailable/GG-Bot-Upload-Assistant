...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            ("S01E09E10", "1", "9", "0", "1", "0"),
            id="multi_episode_release",
        ),
        pytest.param(
            {"date": datetime.date(2022, 4, 12)},
            ("2022-04-12", "0", "0", "0", "0", "1"),
            id="daily_episode",
        ),
    ),
)
...
def __get_torrent_info(bdinfo, raw_file_name, source):
    ...
    if (
        source is not None
    ):  # source will either have a value, or the key itself won't be present
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            __get_torrent_info(
                None,
                "Monsters.at.Work.S01E10.Its.Laughter.They're.After.2160p.WEB-DL.DDP5.1.HDR.H.265-FLUX.mkv",
                "Web",
            ),
            False,  # is_disc
            _get_media_info_video_track(
                f"{working_folder}{mediainfo_xml}Monsters.at.Work.S01E10.Its.Laughter.They're.After.2160p.WEB-DL.DDP5.1.HDR.H.265-FLUX.xml"
            ),
            (None, "PQ10", "H.265", "H.265"),
            id="PQ10_H.265_H.265",
        ),
        pytest.param(
            __get_torrent_info(
                None,
                "The.Great.S02E10.Wedding.2160p.HULU.WEB-DL.DDP5.1.DV.HEVC-NOSiViD.mkv",
                "Web",
            ),
            False,  # is_disc
            _get_media_info_video_track(
            ...
            ("DV", "HDR10+", "H.265", "H.265"),
            id="DV_HDR10+_HEVC",
        ),
        pytest.param(
            __get_torrent_info(
                None,
                "Why.Women.Kill.S02E10.The.Lady.Confesses.2160p.WEB-DL.DD5.1.HDR.HEVC-TEPES.mkv",
                "Web",
            ),
            False,  # is_disc
            _get_media_info_video_track(
            ...
            (None, "HDR10+", "H.265", "H.265"),
            id="HDR10+_HEVC",
        ),
        pytest.param(
            __get_torrent_info(
                None,
                "What.If.2021.S01E01.What.If.Captain.Carter.Were.The.First.Avenger.REPACK.2160p.WEB-DL.DDP5.1.Atmos.DV.HEVC-FLUX.mkv",
                "Web",
            ),
            False,  # is_disc
            _get_media_info_video_track(
        ...
        pytest.param(
            ...
            (None, "HDR", "H.265", "H.265"),
            id="HDR_H.265",
        ),
        pytest.param(
            __get_torrent_info(
                None,
                "Dragon.Booster.S01E01.The.Choosing.Part.1.AMZN.WEB-DL.DDP2.0.H.264-DRAGONE.mkv",
                "Web",
            ),
            False,  # is_disc
            _get_media_info_video_track(
            ...
            (None, None, "H.264", "H.264"),
            id="H.264",
        ),
        pytest.param(
            __get_torrent_info(
                None,
                "Peaky.Blinders.S06E01.Black.Day.2160p.iP.WEB-DL.DDP5.1.HLG.H.265-FLUX.mkv",
                "Web",
            ),
            False,  # is_disc
            _get_media_info_video_track(
            ...
            (None, "HLG", "H.265", "H.265"),
            id="HLG_H.265",
        ),
        pytest.param(
            __get_torrent_info(
                None,
                "Venom.Let.There.Be.Carnage.2021.2160p.UHD.BluRay.REMUX.DV.HDR.HEVC.Atmos-TRiToN.mkv",
                "BluRay",
            ),
            False,  # is_disc
            _get_media_info_video_track(
        ...
        pytest.param(
            __get_torrent_info(
                None,
                "Ran.4K.Remastered.1985.2160p.HDR.UHD-TV.HEVC.AAC-DDR.mkv",
                "BluRay",
            ),
            False,  # is_disc
            _get_media_info_video_track(
...
def test_basic_get_missing_video_codec(
    torrent_info, is_disc, media_info_video_track, expected
):
    assert (
...
def test_basic_get_missing_screen_size(
    torrent_info, is_disc, media_info_video_track, expected
):
    assert (
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            (
                _get_file_contents(
                    f"{working_folder}{mediainfo_summary}Dragon.Booster.S01E01.The.Choosing.Part.1.AMZN.WEB-DL.DDP2.0.H.264-DRAGONE.summary"
                ),
                "0",
                "0",
                "0",
        ...
        pytest.param(
            ...
            (
                _get_file_contents(
                    f"{working_folder}{mediainfo_summary}Venom.Let.There.Be.Carnage.2021.2160p.UHD.BluRay.REMUX.DV.HDR.HEVC.Atmos-TRiToN.summary"
                ),
                "movie/580489",
                "tt7097896",
                "0",
        ...
        pytest.param(
            ...
            (
                _get_file_contents(
                    f"{working_folder}{mediainfo_summary}Peaky.Blinders.S06E01.Black.Day.2160p.iP.WEB-DL.DDP5.1.HLG.H.265-FLUX.summary"
                ),
                "tv/60574",
                "tt2442560",
                "0",
        ...
        pytest.param(
            ...
            (
                _get_file_contents(
                    f"{working_folder}{mediainfo_summary}The.Great.S02E10.Wedding.2160p.HULU.WEB-DL.DDP5.1.DV.HEVC-NOSiViD.summary"
                ),
                "tv/93812",
                "tt2235759",
                "369301",
...
def sample_data_folders_and_files():
    ...
    Path(f"{folder}/raw_file_test_data/tv").mkdir(
        parents=True, exist_ok=True
    )  # raw_file_test_data/tv folder
    Path(f"{folder}/raw_file_test_data/movie").mkdir(
        parents=True, exist_ok=True
    )  # raw_file_test_data/movie folder
    # creating external dupe templates
    shutil.copy(
        f"{working_folder}/tests/resources/media/logo.mp4",
        f"{folder}/raw_file_test_data/movie/logo.mp4",
    )
    shutil.copy(
        f"{working_folder}/tests/resources/media/logo.mp4",
        f"{folder}/raw_file_test_data/tv/logo.S01E01.mp4",
    )
    shutil.copy(
        f"{working_folder}/tests/resources/media/logo.mp4",
        f"{folder}/raw_file_test_data/tv/logo.S01E02.mp4",
    )
    yield
    clean_up(folder)
...
@pytest.mark.parametrize(
    ("upload_media", "expected"),
    [
        pytest.param(
            f"{working_folder}{temp_working_dir}/raw_file_test_data/movie/logo.mp4",
            None,
            id="movie_file",
        ),
        pytest.param(
            f"{working_folder}{temp_working_dir}/raw_file_test_data/movie/",
            f"{working_folder}{temp_working_dir}/raw_file_test_data/movie/logo.mp4",
            id="movie_folder",
        ),
        pytest.param(
        ...
        pytest.param(
            f"{working_folder}{temp_working_dir}/raw_file_test_data/tv",
            f"{working_folder}{temp_working_dir}/raw_file_test_data/tv/logo.S01E01.mp4",
            id="tv_season_pack_folder",
        ),
    ],
)
