...
class GenericUtils:
    ...
    def delete_leftover_files(self, working_folder, file, resume=False):
        """
        ...
        if not Path(
            SCREENSHOTS_PATH.format(base_path=working_folder, sub_folder=unique_hash)
        ).is_dir():
            os.mkdir(
    ...
    def get_and_validate_configured_trackers(
        self, trackers, all_trackers, api_keys_dict, all_trackers_list
    ):
        upload_to_trackers = []
        # small sanity check
        ...
        if "PTP" in upload_to_trackers:
            ...
            if not self._can_upload_to_ptp():
                logging.error(
    ...
    def _post_mode_cross_seed(
        ...
        if (
            ...
            and torrent_info[f"{tracker}_upload_status"] is True
        ):
            logging.info(
    ...
    def perform_post_processing(
        ...
        if upload_assistant_config.ENABLE_POST_PROCESSING:
            ...
            translated_path = self._get_client_translated_path(torrent_info)
            if translated_path is False:
                return False
            ...
            if post_processing_mode == "CROSS_SEED":
                ...
                return self._post_mode_cross_seed(
                    torrent_client,
                    torrent_info,
                    working_folder,
                    tracker,
                ...
                return self._post_mode_watch_folder(torrent_info, working_folder)
            else:
                logging.error(
    ...
    def validate_and_load_external_templates(self, template_validator, working_folder):
        """
    ...
    def normalize_for_system_path(file_path: str) -> str:
        ...
        return re.sub(r"[-\s]+", "_", re.sub(r"[^\w\s-]", "", file_path.lower())).strip(
            "__"
        )

    def validate_batch_mode(
        self, *, batch_mode: bool, path: List[str], metadata_ids: Dict[str, str]
    ) -> bool:
        if not batch_mode:
            return True
        ...
        if len(path) != 1 or not os.path.isdir(path[0]):
            if len(path) != 1:
                self._log_error_and_exit_batch_and_multiple_path()
            else:
                self._log_error_and_exit_batch_and_file_path()
