...
class BasicUtils:
    ...
    def quit_log_reason(reason, missing_value):
        ...
        console.print(
            f"\nCritical error when trying to extract: {missing_value}",
            style="red bold",
        )
    ...
    def _get_dv_hdr(media_info_video_track):
        ...
        try:
            logging.debug(
                "[BasicUtils] Logging video track atrtibutes used to detect HDR type"
            )
            logging.debug(
            ...
            if color_primaries is not None and color_primaries in (
                "BT.2020",
                "REC.2020",
            ):
                ...
                if "HDR10+" in hdr_format:
                    hdr = "HDR10+"
                elif media_info_video_track.hdr_format is None and "PQ" in (
                    media_info_video_track.transfer_characteristics,
                    media_info_video_track.transfer_characteristics_original,
                ):
                    hdr = "PQ10"

                if (
                    media_info_video_track.transfer_characteristics_original is not None
                    and "HLG"
                    in media_info_video_track.transfer_characteristics_original
                ):
                    hdr = "HLG"
                elif (
                    media_info_video_track.transfer_characteristics_original is not None
            ...
            logging.exception(
                "[BasicUtils] Error occurred while trying to parse HDR information from mediainfo."
            )

        if (
            media_info_video_track.hdr_format is not None
            and "Dolby Vision" in media_info_video_track.hdr_format
        ):
            dv = "DV"
    ...
    def basic_get_missing_video_codec(
        self, torrent_info, is_disc, auto_mode, media_info_video_track
    ):
        ...
        # First try to use our own Regex to extract it, if that fails then we can use ffprobe/mediainfo
        filename_video_codec_regex = re.search(
            r"(?P<HEVC>HEVC)|(?P<AVC>AVC)|"
            r"(?P<H265>H(.265|265))|"
            r"(?P<H264>H(.264|264))|"
        ...
        if "HEVC" in media_info_video_track.format:
            ...
            if media_info_video_track.writing_library is not None:
                logging.debug(
                    "[BasicUtils] Writing library is present in track. Hence using x265 as codec"
                )
            ...
            if "source" in torrent_info and torrent_info["source"] == "Web":
                logging.debug(
                    "[BasicUtils] Video source is 'Web, Hence overriding video codec to H.265"
                )
        ...
        # Now check and assign AVC based codecs
            ...
            if media_info_video_track.writing_library is not None:
                logging.debug(
                    "[BasicUtils] Writing library is present in track. Hence using x264 as codec"
                )
            ...
            if "source" in torrent_info and torrent_info["source"] == "Web":
                logging.debug(
                    "[BasicUtils] Video source is 'Web, Hence overriding video codec to H.264"
                )
        ...
        return (
            ...
            hdr,
            regex_video_codec
            if regex_video_codec is not None
            else pymediainfo_video_codec,
    ...
    def basic_get_missing_audio_codec(
        self,
        torrent_info,
        is_disc,
        auto_mode,
        audio_codec_file_path,
        media_info_audio_track,
        parse_me,
        missing_value,
    ):
        ...
        atmos = self.__get_atmos_from_media_info(media_info_audio_track)
        ...
        if "audio_codec" in torrent_info:
            ...
            for key in audio_codec_dict.keys():
                if str(torrent_info["audio_codec"]) == key:
                    ...
                    return audio_codec_dict[torrent_info["audio_codec"]], atmos
        ...
        bluray_disc_audio_codec_regex = re.search(
            r"(?P<TrueHD>TrueHD)|(?P<DTSHDMA>DTS-HD.MA)",
            torrent_info["raw_file_name"].replace(".", " "),
            re.IGNORECASE,
        )
        ...
        if audio_codec:
            if "AAC" in audio_codec:
                # AAC gets its own 'if' statement because 'audio_codec' can return something like 'AAC LC-SBR' or
                # 'AAC-HE/LC' It's unnecessary for a torrent title, and we only need the "AAC" part
                logging.info(
            ...
            if "FLAC" in audio_codec:
                ...
                logging.info(
                    "[BasicUtils] Used pymediainfo to identify the audio codec: FLAC"
                )
                return "FLAC", atmos
            ...
            if "DTS" in audio_codec:
                ...
                match_dts_audio = re.search(
                    r"DTS(-HD(.MA.)|-ES.|(.[xX].|[xX].)|(.HD.|HD.)|)",
                    torrent_info["raw_file_name"].replace(" ", "."),
                    re.IGNORECASE,
                )
                if match_dts_audio is not None:
                    ...
                    return_value = (
                        str(match_dts_audio.group()).upper().replace(".", " ").strip()
                    )
                    ...
                    if return_value in audio_codec_dict.keys():
                        # Now it's a bit of a Hail Mary, and we try to match whatever pymediainfo returned to our
                        # audio_codec_dict/translation
                        logging.info(
                            f"[BasicUtils] Used (pymediainfo + regex + audio_codec_dict) to identify the audio codec: {audio_codec_dict[return_value]}"
                        )
                        return audio_codec_dict[return_value], atmos
                    return return_value, atmos
        ...
        if UploadAssistantConfig().FORCE_AUTO_MODE:
            logging.info(
        ...
        # Exit the script now
        self.quit_log_reason(
            reason="Could not detect audio_codec via regex, pymediainfo, & ffprobe. force_auto_upload=false so we quit now",
            missing_value=missing_value,
        )

    def basic_get_missing_audio_channels(
        self,
        torrent_info,
        is_disc,
        auto_mode,
        ...
        for stream in audio_info["streams"]:
            ...
            if "channel_layout" in stream:
                ...
                if str(stream["channel_layout"]) in ffmpy_channel_layout_translation:
                    stream["channel_layout"] = ffmpy_channel_layout_translation[
                        stream["channel_layout"]
                    ]

                # Make sure what we got back from the ffprobe search fits into the audio_channels 'format' (num.num)
                audio_channel_layout = re.search(
                    r"\d\.\d", str(stream["channel_layout"]).replace("(side)", "")
                )
        ...
        # Exit the script now
        self.quit_log_reason(
            reason="Audio_Channels are not in the filename, and we can't extract it using regex or ffprobe. force_auto_upload=false so we quit now",
            missing_value=missing_value,
        )

    def basic_get_missing_screen_size(
        self, torrent_info, is_disc, media_info_video_track, auto_mode, missing_value
    ):
        width_to_height_dict = {
            "720": "576",
            "960": "540",
            "1280": "720",
            "1920": "1080",
        ...
        if is_disc and torrent_info["bdinfo"] is not None:
            logging.info(
                f"[BasicUtils] `screen_size` identified from bdinfo as {torrent_info['bdinfo']['video'][0]['resolution']}"
            )
        ...
        # User input as a last resort
            ...
            if not auto_mode:
                ...
                self.quit_log_reason(
                    reason="Resolution not in filename, and we can't extract it using pymediainfo. Upload form requires the Resolution",
                    missing_value=missing_value,
                )

    def basic_get_missing_source(self, torrent_info, is_disc, auto_mode, missing_value):
        ...
        if not auto_mode:
            console.print(
                f"Can't auto extract the [bold]{missing_value}[/bold] from the filename, you'll need to manually specify it",
                style="red",
                highlight=False,
            )
            ...
            source = Prompt.ask(
                ...
                choices=["bluray", "web", "hdtv", "dvd", "pdtv", "sdtv"],
            )
            ...
            self.quit_log_reason(
                reason="auto_mode is enabled & we can't auto detect the source (e.g. bluray, webdl, dvd, etc). Upload form requires the Source",
                missing_value=missing_value,
            )

    def basic_get_missing_mediainfo(self, torrent_info, parse_me, mediainfo_file):
        ...
        if "largest_playlist" not in torrent_info:
            ...
            media_info_output = str(
                MediaInfo.parse(parse_me, output="text", full=False)
            ).replace(parse_me, essential_path)
    ...
    def basic_get_raw_video_file(self, upload_media):
        raw_video_file = None
    ...
    def basic_get_episode_basic_details(self, guess_it_result):
        season_number = "0"
        episode_number = "0"
        complete_season = "0"
        ...
        if "season" not in guess_it_result:
            ...
            if (
                "date" in guess_it_result
            ):  # we can replace the S**E** format with the daily episodes date
                daily_episode_date = str(guess_it_result["date"])
                logging.info(
                    f"[BasicUtils] Detected a daily episode, using the date ({daily_episode_date}) instead of S**E**"
                )
    ...
    def get(_this, _or, _from, _default=""):
        return (
            _from[_this][0]
            if _this is not None and _this in _from
            else _from[_or]
            if _or is not None and _or in _from
            else _default
        )

    def prepare_mediainfo_summary(self, media_info_result):
        ...
        for track in media_info_result["tracks"]:
            if track["track_type"] == "General":
                general = dict()
                general["Container"] = self.get(
                    _this="other_format", _or="format", _from=track
                )
                general["Size"] = self.get(
                    _this="other_file_size", _or="file_size", _from=track
                )
                general["Duration"] = self.get(
                    _this="other_duration", _or="duration", _from=track
                )
                general["Bit Rate"] = self.get(
                    _this="other_overall_bit_rate",
                    _or="overall_bit_rate",
                    _from=track,
                )
                general["Frame Rate"] = self.get(
                    _this="other_frame_rate", _or="frame_rate", _from=track
                )
                general["tmdb"] = self.get(
                    _this=None, _or="tmdb", _from=track, _default="0"
                )
                general["imdb"] = self.get(
                    _this=None, _or="imdb", _from=track, _default="0"
                )
                general["tvdb"] = self.get(
                    _this=None, _or="tvdb", _from=track, _default="0"
                )
                mediainfo_summary["General"] = general
            elif track["track_type"] == "Video":
                video = dict()
                video["Codec"] = self.get(
                    _this="other_format", _or="format", _from=track
                )
                video["Bit Rate"] = self.get(
                    _this="other_bit_rate", _or="bit_rate", _from=track
                )
                video["Frame Rate"] = self.get(
                    _this="other_frame_rate", _or="frame_rate", _from=track
                )
                video["Bit Depth"] = self.get(
                    _this="other_bit_depth", _or="bit_depth", _from=track
                )
                video["Language"] = self.get(
                    _this="other_language", _or="language", _from=track
                )
                video["Aspect Ratio"] = self.get(
                    _this="other_display_aspect_ratio",
                    _or="display_aspect_ratio",
                    _from=track,
                )
                video["Resolution"] = (
                    f'{self.get(_this="sampled_width", _or="width", _from=track)}x{self.get(_this="sampled_height", _or="height", _from=track)}'
                )
                mediainfo_summary["Video"].append(video)
            elif track["track_type"] == "Audio":
                audio = dict()
                audio["Format"] = self.get(
                    _this="other_commercial_name",
                    _or="commercial_name",
                    _from=track,
                )
                audio["Channels"] = self.get(
                    _this="other_channel_s", _or="channel_s", _from=track
                )
                audio["Sampling Rate"] = self.get(
                    _this="other_sampling_rate", _or="sampling_rate", _from=track
                )
                audio["Compression"] = self.get(
                    _this="other_compression_mode",
                    _or="compression_mode",
                    _from=track,
                )
                audio["Language"] = self.get(
                    _this="other_language", _or="title", _from=track
                )
                audio["Bit Rate"] = self.get(
                    _this="other_bit_rate", _or="bit_rate", _from=track
                )
                audio["Bit Rate Mode"] = self.get(
                ...
                text["Language"] = self.get(
                    _this="other_language", _or="title", _from=track
                )
                text["Title"] = track["title"] if "title" in track else ""
                text["Forced"] = track["Forced"] if "Forced" in track else ""
                # we need the ISO 3 or 2-letter code for the language. We can get it from `language` or find it from
                # `other_language`
                if "language" in track:
                    ...
                    text["language_code"] = [
                        lang_code
                        for lang_code in track["other_language"]
        ...
        summary = "---(GENERAL)----" + "\n"
        summary += "\n".join(
            f'{key.ljust(15, ".")}: {value}'
            for key, value in mediainfo_summary["General"].items()
        )
        ...
        if len(mediainfo_summary["Text"]) > 0:
            ...
            summary += "--(SUBTITLES)---" + "\n"
            summary += (
                f'{"Format".ljust(15, ".")}: {mediainfo_summary["Text"][0]["Format"]}'
                + "\n"
            )
            summary += f'{"Language".ljust(15, ".")}: '
            for text_track in mediainfo_summary["Text"]:
                text_track = {
                    k: v for k, v in text_track.items() if k.startswith("Language")
                }
