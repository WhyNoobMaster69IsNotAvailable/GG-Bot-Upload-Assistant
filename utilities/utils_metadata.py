...
def __is_auto_reuploader():
    return UploaderConfig().REUPLOADER


def _return_for_reuploader_and_exit_for_assistant(
    selected_tmdb_results_data=None,
):
...
def _metadata_search_tmdb_for_id(query_title, year, content_type, auto_mode):
    ...
    if search_tmdb_request.ok:
        ...
        if len(search_tmdb_request.json()["results"]) == 0:
            logging.critical(
                "[MetadataUtils] No results found on TMDB using the title '{}' and the year '{}'".format(
            ...
            if search_tmdb_request.ok:
                if len(search_tmdb_request.json()["results"]) == 0:
                    logging.critical(
                        f"[MetadataUtils] No results found on TMDB using the title '{query_title}' and the year '{year}'"
                    )
        ...
        logging.info(
            "[MetadataUtils] TMDB search has returned proper responses. Parsing and identifying the proper TMDB Id"
        )
        logging.info(
            f"[MetadataUtils] TMDB Search parameters. Title :: {query_title}, Year :: '{year}'"
        )
        ...
        for possible_match in search_tmdb_request.json()["results"]:
            ...
            title_match = list(
                map(
                    possible_match.get,
                    filter(lambda x: x in "title, name", possible_match),
                )
            )
            ...
            year_match = list(
                map(
                    possible_match.get,
                    filter(
                        lambda x: x in "release_date, first_air_date",
                        possible_match,
                    ),
                )
            )
            if len(year_match) > 0:
                ...
                logging.error(
                    f"[MetadataUtils] Year not found on TMDB for TMDB ID: {str(possible_match['id'])}"
                )
            ...
            if content_type == "tv":
                logging.info(
                    "[MetadataUtils] Skipping year matching since this is an episode."
                )
            elif (
                year != ""
                and int(year) > 0
                and selected_year != "N.A."
                and len(selected_year) > 0
            ):
                year = int(year)
                selected_year_sub_part = int(selected_year.split("-")[0])
                logging.info(
                    f"[MetadataUtils] Applying year filter. Expected years are [{year - 1}, {year}, or {year + 1}]. Obtained year [{selected_year_sub_part}]"
                )
                if (
                    selected_year_sub_part == year
                    or selected_year_sub_part == year - 1
                    or selected_year_sub_part == year + 1
                ):
                    logging.info(
                        "[MetadataUtils] The possible match has passed the year filter"
                    )
                else:
                    logging.info(
                        "[MetadataUtils] The possible match failed to pass year filter."
                    )
            ...
            if "overview" in possible_match:
                if len(possible_match["overview"]) > 1:
                    ...
                    logging.error(
                        f"[MetadataUtils] Overview not found on TMDB for TMDB ID: {str(possible_match['id'])}"
                    )
                    overview = "N.A."
            else:
                overview = "N.A."
        ...
        logging.info(
            f"[MetadataUtils] Total number of results for TMDB search: {str(result_num)}"
        )
        if result_num < 1:
            console.print(
                "Cannot auto select a TMDB id. Marking this upload as [bold red]TMDB_IDENTIFICATION_FAILED[/bold red]"
            )
            logging.info(
        ...
        if __is_auto_reuploader():
            ...
            if (
                ...
                logging.info(
                    f"[MetadataUtils] `tmdb_result_auto_select_threshold` is valid so we are auto selecting #1 from "
                    f"tmdb results (TMDB ID: {str(result_dict[user_input_tmdb_id_num])})"
                )
            else:
                console.print(
                    "Cannot auto select a TMDB id. Marking this upload as [bold red]TMDB_IDENTIFICATION_FAILED[/bold "
                    "red]"
                )
                logging.info(
                    "[MetadataUtils] Cannot auto select a TMDB id. Marking this upload as TMDB_IDENTIFICATION_FAILED"
            ...
            if auto_mode or selected_tmdb_results == 1:
                ...
                user_input_tmdb_id_num = "1"
                logging.info(
                    f"[MetadataUtils] 'auto_mode' is enabled or 'only 1 result from TMDB', so we are auto selecting #1 from tmdb results (TMDB ID: {str(result_dict[user_input_tmdb_id_num])})"
                )
            else:
                # prompt for user input with 'list_of_num' working as a list of valid choices
                user_input_tmdb_id_num = Prompt.ask(
        ...
        if content_type in ["episode", "tv"]:  # getting TVmaze ID
            ...
            if tmdb_external_ids["imdb"] != "0":
                tmdb_external_ids["tvmaze"] = str(
                    _get_external_id(
                        id_site="imdb",
                        id_value=tmdb_external_ids["imdb"],
                        external_site="tvmaze",
                        content_type=content_type,
                    )
                )
                if (
                    tmdb_external_ids["tvmaze"] == "0"
                    and tmdb_external_ids["tvdb"] != "0"
                ):
                    tmdb_external_ids["tvmaze"] = str(
                        _get_external_id(
                            id_site="tvdb",
                            id_value=tmdb_external_ids["tvdb"],
                            external_site="tvmaze",
                            content_type=content_type,
                        )
                    )
    ...
    try:
        if external_site == "tvmaze":  # we need tvmaze id
            ...
            if id_site == "imdb":  # we have imdb id
                ...
                return (
                    str(tvmaze_id_request["id"])
                    if tvmaze_id_request["id"] is not None
                    else "0"
                )
            elif id_site == "tvdb":  # we have tvdb id
                logging.info(
                    f"[MetadataUtils] GET Request For TVMAZE Lookup: {tvmaze_id_from_tvdb}"
                )
                ...
                logging.error(
                    f"[MetadataUtils] We cannot get {external_site} from {id_site}. Returning '0' as response"
                )
                return "0"
            ...
            if id_site == "imdb":  # we have imdb id
                logging.info(
                    f"[MetadataUtils] GET Request For TMDB Lookup: {tmdb_id_from_imdb_redacted}"
                )
                tmdb_id_request = requests.get(tmdb_id_from_imdb).json()
            elif id_site == "tvdb":  # we have tvdb id
                logging.info(
                    f"[MetadataUtils] GET Request For TMDB Lookup: {tmdb_id_from_tvdb_redacted}"
                )
            ...
            if tmdb_id_request is not None:
                if content_type == "tv":
                    if len(tmdb_id_request["tv_results"]) == 1:
                        logging.info(
                            f"[MetadataUtils] Returning tmdb id as `{str(tmdb_id_request['tv_results'][0]['id'])}`"
                        )
                        return str(tmdb_id_request["tv_results"][0]["id"])
                else:
                    if len(tmdb_id_request["movie_results"]) == 1:
                        logging.info(
                            f"[MetadataUtils] Returning tmdb id as `{str(tmdb_id_request['movie_results'][0]['id'])}`"
                        )
                        return str(tmdb_id_request["movie_results"][0]["id"])
    except Exception as e:
        logging.exception(
            f"[MetadataUtils] Error while fetching {external_site} from {id_site}. Returning `0` as the response",
            exc_info=e,
        )

    logging.info(
        f"[MetadataUtils] Returning fall back value of '0' for fetching id for {external_site} from {id_site} "
    )
    return "0"
...
def search_for_mal_id(content_type, tmdb_id, torrent_info):
    ...
    temp_map = {"tvdb": 0, "mal": 0, "tmdb": tmdb_id}
    if content_type == "tv":
        get_tvdb_id = f"https://api.themoviedb.org/3/tv/{tmdb_id}/external_ids?api_key={UploaderConfig().TMDB_API_KEY}&language=en-US"
        logging.info(
            f"[MetadataUtils] GET Request For TVDB Lookup: https://api.themoviedb.org/3/tv/{tmdb_id}/external_ids?api_key=<REDACTED>&language=en-US"
        )
        get_tvdb_id_response = requests.get(get_tvdb_id).json()
        # Look for the tvdb_id key
        if (
            "tvdb_id" in get_tvdb_id_response
            and get_tvdb_id_response["tvdb_id"] is not None
        ):
...
def _fill_tmdb_metadata_to_torrent_info(torrent_info, tmdb_response):
    ...
    tmdb_metadata["tags"] = list(
        map(
            lambda genre: genre.lower().replace(" ", "").replace("-", ""),
            tmdb_metadata["genres"],
        )
    )
...
def _fill_imdb_metadata_to_torrent_info(
    ...
    if datasource not in ["API", "CINEMAGOER"]:
        ...
        logging.error(
            f"[MetadataUtils] IMDb metadata from invalid source {datasource} provided. Skipping filling IMDb metadata."
        )
        return
    ...
    imdb_metadata["tags"] = list(
        map(
            lambda genre: genre.lower().replace(" ", "").replace("-", ""),
            imdb_metadata["genres"],
        )
    )
        ...
        if keywords_info is not None and "status_message" not in keywords_info:
            ...
            torrent_info["tmdb_metadata"]["keywords"] = list(
                map(
                    ...
                    keywords_info[keywords_attribute],
                )
            )
            logging.info(
                f"[MetadataUtils] Obtained the following keywords from TMDb :: {torrent_info['tmdb_metadata']['keywords']}"
            )
        else:
            logging.error(
                "[MetadataUtils] Could not obtain keywords for the release from TMDb"
            )
    except Exception as e:
        logging.exception(
            "[MetadataUtils] Error occurred while trying to fetch keywords for the release.",
            exc_info=e,
        )
        ...
        if trailers_info is not None and "status_message" not in trailers_info:
            ...
            torrent_info["tmdb_metadata"]["trailer"] = list(
                map(
                    ...
                    filter(
                        lambda entry: entry["type"] == "Trailer"
                        and entry["site"] == "YouTube",
                        trailers_info["results"],
                    ),
                )
            )
            logging.info(
                f"[MetadataUtils] Obtained the following trailers from TMDb :: {torrent_info['tmdb_metadata']['trailer']}"
            )
        else:
            logging.error(
                "[MetadataUtils] Could not obtain trailers for the release from TMDb"
            )
    except Exception as e:
        logging.exception(
            "[MetadataUtils] Error occurred while trying to fetch trailers for the release.",
            exc_info=e,
        )


def metadata_compare_tmdb_data_local(torrent_info):
    # We need to use TMDB to make sure we set the correct title & year as well as correct punctuation, so we don't get
    # held up in torrent moderation queues I've outlined some scenarios below that can trigger issues if we just try
    # to copy and paste the file name as the title

    # 1. For content that is 'non-english' we typically have a foreign title that we can (should) include in the
    # torrent title using 'AKA' (K so both TMDB & OMDB API do not include this info, so we aren't doing this)
    #
    # 2. Some content has special characters (e.g.  The Hobbit: An Unexpected Journey   or   Welcome,
    # or No Trespassing  ) we need to include these in the torrent title
    #
    # 3. For TV Shows, Scene groups typically don't include the episode title in the filename, but we get this info
    # from TMDB and include it in the title 4. Occasionally movies that have a release date near the start of a new
    # year will be using the incorrect year ( e.g. the movie '300 (2006)' is occasionally mislabeled as '300 (2007)'
    #
    # This will run regardless is auto_mode is set to true or false since I consider it pretty important to comply
    # with all site rules and avoid creating extra work for tracker staff
    ...
    # TODO: in most cases if tmdb doesn't have the trailer information, them imdb also won't have it.
...
def _get_external_ids_from_imdb(imdb):
    imdb_api_key = UploaderConfig().IMDB_API_KEY
    if imdb_api_key is None:
        logging.info(
            "[MetadataUtils] IMDB Api is not enabled. Skipping metadata fetch from imdb."
        )
        return None

    logging.info("[MetadataUtils] Fetching external ids from IMDB")
    imdb_external_id_url = (
        f"https://imdb-api.com/API/ExternalSites/{imdb_api_key}/{imdb}"
    )
    ...
    try:
        ...
        if len(imdb_response["errorMessage"]) > 0:
            logging.error(
                f"[MetadataUtils] Error obtained from imdb api. Error '{imdb_response['errorMessage']}' "
            )
            # we couldn't get any data from imdb api. Possibly invalid imdb id
            return None

        tvdb_data = (
            imdb_response["theTVDB"]["id"]
            if imdb_response["theTVDB"] is not None
            else "0"
        )
        tmdb_data = (
            imdb_response["theMovieDb"]["id"]
            if imdb_response["theMovieDb"] is not None
            else "0"
        )
        ...
        logging.exception(
            "[MetadataUtils] Fatal error occurred while fetching external ids from imdb api.",
            exc_info=e,
        )
        return None
...
def _get_external_ids_from_tmdb(content_type, tmdb):
    ...
    try:
        ...
        if "status_message" in tmdb_response:
            logging.error(
                f"[MetadataUtils] Error obtained from tmdb api. Error '{tmdb_response['status_message']}' "
            )
        ...
        logging.exception(
            "[MetadataUtils] Fatal error occurred while fetching external ids from TMDB api.",
            exc_info=e,
        )
        return None
...
def _get_external_ids_from_tvmaze(tvmaze):
    ...
    try:
        ...
        if "message" in tvmaze_response:
            logging.error(
                f"[MetadataUtils] Error obtained from tvmaze api. Error '{tvmaze_response['name']}' "
            )
        ...
        logging.exception(
            "[MetadataUtils] Fatal error occurred while fetching external ids from TVMAZE api.",
            exc_info=e,
        )
        return None
...
def fill_database_ids(
    torrent_info, tmdb_id, imdb_id, tvmaze_id, auto_mode, tvdb_id=None
):
    possible_matches = None
    metadata_providers = ["imdb", "tmdb", "tvmaze", "tvdb"]
    # small sanity check
    # TODO: Should the mal id collected in arguments be used ??
    tmdb_id, imdb_id, tvmaze_id, tvdb_id = _sanitize_metadata_from_arguments(
        tmdb_id, imdb_id, tvmaze_id, tvdb_id
    )
    ...
    for media_id_key, media_id_val in {
        "tmdb": tmdb_id,
        "imdb": imdb_id,
        "tvmaze": tvmaze_id,
        "tvdb": tvdb_id,
    ...
    if all(
        ...
        for _ in range(0, 2):
            ...
            if "imdb" in ids_present and any(
                ...
                _fill_ids_from_external_response(
                    _get_external_ids_from_imdb(torrent_info["imdb"]),
                    torrent_info,
                    ids_missing,
                    ids_present,
                )
            ...
            if "tmdb" in ids_present and any(
                ...
                _fill_ids_from_external_response(
                    _get_external_ids_from_tmdb(
                        torrent_info["type"], torrent_info["tmdb"]
                    ),
                    torrent_info,
                    ids_missing,
                    ids_present,
            ...
            if torrent_info["type"] == "episode":
                ...
                if "tvdb" in ids_present:
                    ...
                    if "tmdb" in ids_missing:
                        torrent_info["tmdb"] = _get_external_id(
                            id_site="tvdb",
                            id_value=torrent_info["tvdb"],
                            external_site="tmdb",
                    ...
                    if "tvmaze" in ids_missing:
                        torrent_info["tvmaze"] = _get_external_id(
                            id_site="tvdb",
                            id_value=torrent_info["tvdb"],
                            external_site="tvmaze",
                            content_type=torrent_info["type"],
        ...
        logging.info(
            "[MetadataUtils] Finished fetching external ids from the provided ids. Information collected so far..."
        )
        ...
        logging.info(
            "[MetadataUtils] Attempting to resolve any ids that are still missing."
        )

        if "imdb" in ids_missing:
            # we couldn't get imdb id. and we cannot get it
            logging.fatal(
                "[MetadataUtils] Could not resolve IMDB id. If IMDB Id is mandatory, then it needs to be provided via runtime argument '--imdb'"
            )

        if "tvdb" in ids_missing and torrent_info["type"] == "episode":
            # we couldn't get tvdb id, and we cannot get it
            logging.fatal(
                "[MetadataUtils] Could not resolve TVDB id. If TVDB Id is mandatory, then it needs to be provided via runtime argument '--tvdb'"
            )
        ...
        logging.info(
            "[MetadataUtils] Metadata Information collected after individual searches..."
        )
        logging.info(f'[MetadataUtils] IMDB ID: {torrent_info["imdb"]}')
        logging.info(f'[MetadataUtils] TMDB ID: {torrent_info["tmdb"]}')
        logging.info(f'[MetadataUtils] TVMAZE ID: {torrent_info["tvmaze"]}')
...
def _search_and_get_possible_matches(torrent_info, auto_mode):
    ...
    # this method searches and gets all three ids ` 'imdb', 'tmdb', 'tvmaze' `
    metadata_result = _metadata_search_tmdb_for_id(
        query_title=torrent_info["title"],
...
def _fill_ids_from_external_response(
    external_ids_resolved, torrent_info, ids_missing, ids_present
):
