...
class MiscellaneousUtils:
    ...
    def identify_bluray_disc_type(
        ...
        if test_size is not None:
            ...
            total_size = sum(
                f.stat().st_size for f in Path(upload_media).glob("**/*") if f.is_file()
            )
    ...
    @staticmethod
    def identify_repacks(raw_file_name) -> Optional[str]:
    ...
    @staticmethod
    def identify_web_streaming_source(
        streaming_services,
    ...
    @staticmethod
    def identify_source_type(raw_file_name, auto_mode, source):
        logging.debug(
        ...
        # Well firstly if we got this far with auto_mode enabled that means we've somehow figured out the 'parent'
        # source but now can't figure out its 'final form' If auto_mode is disabled we can prompt the user
        elif not auto_mode:
            ...
            # Since we already know the 'parent source' from an earlier function we don't need to prompt the user for
            # it twice
            if str(source).lower() in basic_source_to_source_type_dict and isinstance(
                ...
                # The user is given a list of options that are specific to the parent source they choose earlier (
                # e.g.  bluray --> disc, remux, encode )
        ...
        # Well this sucks, we got pretty far this time but since 'auto_mode=true' we can't prompt the user & it
        # probably isn't a great idea to start making assumptions about a media files source, that seems like a good
        # way to get a warning/ban so instead we'll just quit here and let the user know why
        else:
            logging.critical(
            ...
            # and finally exit since this will affect all trackers we try and upload to, so it makes no sense to try
            # the next tracker
            sys.exit()
        logging.debug(
    ...
    def fill_dual_multi_and_commentary(original_language, audio_tracks):
        ...
        for audio_track in audio_tracks:
            ...
            if original_language != "en" and original_language != "":
                ...
                # check for original
                if not original:
                    if audio_language == original_language and "commentary" not in (
                        audio_track.title.lower()
                        if audio_track.title is not None
                        else ""
                    ):
    ...
    def detect_anamorphic_video_and_pixel_ratio(video_track):
        ...
        try:
            pixel_aspect_ratio = float(f"{float(video_track.pixel_aspect_ratio):.3f}")
        except ValueError:
            logging.error(
                "[MiscellaneousUtils] Failed to get proper pixel aspect ratio for detecting anamorphic video"
            )
            logging.info(
    ...
    @staticmethod
    def perform_scene_group_capitalization(scene_groups_path, torrent_info):
        ...
        try:
            ...
            logging.fatal(
                "[MiscellaneousUtils] Error from pre corroupt db.", exc_info=ex
            )
        ...
        if (
            ...
            try:
                ...
                logging.fatal("[MiscellaneousUtils] Error from srrdb.", exc_info=ex)
                srrdb_response = {}
