...
def _load_components(custom_text_components_path):
    ...
    components_choices = ", ".join(
        map(lambda component: f'\'{component["key"]}\'', custom_text_components)
    )
    logging.debug(
        f"[CustomUserInputs] keys configured in the bot :: {components_choices}"
    )
...
def add_item_to_custom_texts(
    custom_text_components_path, user_custom_texts, item, text="", header=""
):
    if text == "" and header == "":
...
def collect_custom_messages_from_user(custom_text_components_path):
    logging.debug(
        "[CustomUserInputs] Starting to collect custom messages from user for torrent description"
    )

    user_custom_texts = []
    custom_text_components, components_choices = _load_components(
        custom_text_components_path
    )
    # creating the choices list for user
    list_of_num = _create_choice_list_for_user(custom_text_components)

    console.rule(
        "[bold red] :warning: Custom Torrent Descriptions! :warning: [/bold red]",
        style="bold red",
        align="center",
    )
    console.print(
        f"You can add custom messages and texts to the torrent description as various text components. These could be {components_choices}",
        justify="center",
    )
    console.print(
        "[bold red]:warning: Please note that only few trackers supports all these different types. :warning:[/bold red]",
        highlight=False,
        justify="center",
    )
    console.print(
        "If any tracker does not support one of the type, then it will be defaulted to 'PLAIN TEXT'",
        justify="center",
    )
    console.print(
        "[bold red]:warning: The text components added will be added to torrent description in the order which they are entered :warning:[/bold red]",
        justify="center",
    )
    ...
    while keep_going_mate:
        ...
        if choice == str(len(custom_text_components)):
            console.print(
                "[bold blue]:facepunch: Yes... Yes you are.:facepunch: [/bold blue] [bold red]Skipping further custom torrent descriptions collection and proceeding with upload[/bold red] :angry_face: ",
                justify="center",
            )
            ...
            if custom_text_components[int(choice) - 1]["title"]:
                console.print(
                    "Please provide the title for the component ([bold green]One Liner[/bold green])"
                )
                title = input("::").split("\\n")[0]
                logging.debug(
                    f'[CustomUserInputs] User provided {title} as title for {custom_text_components[int(choice) - 1]["key"]}'
                )

            console.print(
                f"Please provide the contents for '{custom_text_components[int(choice) - 1]['key']}'. [bold red on white] Send EOF or Ctrl + D from new line to stop text grabbing [/bold red on white]"
            )
            custom_text = sys.stdin.read()
            console.print(
                "[green] ----------------- USER INPUT COLLECTED ----------------- [/green]"
            )
            ...
            user_custom_texts.append(
                {
                    "key": custom_text_components[int(choice) - 1][
                        "tracker_reference_key"
                    ],
                    "value": custom_text,
                    "title": title,
    ...
    logging.debug(
        f"[CustomUserInputs] Custom text components collected :: {pformat(user_custom_texts)}"
    )
    return user_custom_texts
